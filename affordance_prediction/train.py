...
from affordancedata import ToTensorBCHW, GrayAffordancesDataset, RandomCrop, RandomFlip, RandomRotate, TensorAffordancesDataset, ImageAffordancesDataset
...
class Trainer(object):
    def __init__(self, args):
        ...
        # crop_data = GrayAffordancesDataset(image_dir="data/train_img/", affordances_dir="data/train_label/", transform=transforms.Compose([RandomCrop((args.crop_size,args.crop_size)), input_transform]))
        # flip_rotate_data = GrayAffordancesDataset(image_dir="data/train_img/", affordances_dir="data/train_label/", transform=transforms.Compose([RandomFlip(flip_left_right=True, flip_top_bottom=True), RandomRotate(180), input_transform]))
        # pure_data = GrayAffordancesDataset(image_dir="data/train_img/", affordances_dir="data/train_label/", transform=transforms.Compose([ input_transform]))
        # train_dataset = ConcatDataset([pure_data, flip_rotate_data, crop_data])
        # while len(train_dataset) < args.num_images:
        #     train_dataset = ConcatDataset([train_dataset, flip_rotate_data, crop_data])

        # aug_dataset = TensorAffordancesDataset(
        #     image_dir='data/aug_img/', affordances_dir='data/aug_label/')
        # aug_dataset = Subset(aug_dataset, range(args.num_images))
        aug_dataset = GrayAffordancesDataset(
        ...
        #test_dataset = GrayAffordancesDataset(image_dir="data/test_img/", affordances_dir="data/test_label/", transform=transforms.Compose([ input_transform]))
        ...
        args.iters_per_epoch = len(
            aug_dataset) // (args.num_gpus * args.batch_size)
        ...
        self.train_loader = DataLoader(dataset=aug_dataset,
                                       shuffle=True,
                                       batch_size=args.batch_size,
                                       num_workers=args.workers,
                                       pin_memory=True)
        self.test_loader = DataLoader(dataset=aug_dataset,
                                      shuffle=False,
                                      batch_size=1,
                                      num_workers=args.workers,
                                      pin_memory=True)
        ...
        #print('weights: {}'.format(pos_weight[:, 0, 0].data))
